...
app = Flask(__name__)
app.secret_key = os.getenv('SECRET_KEY', 'rahasia')

# ================== DATABASE ==================
DB_PATH = os.getenv('DB_PATH', '/data/sma.db')
...
UPLOAD_FOLDERS = {
    'galeri': os.path.join(UPLOAD_BASE, 'galeri'),
    'guru': os.path.join(UPLOAD_BASE, 'guru'),
    'alumni': os.path.join(UPLOAD_BASE, 'alumni'),
    'ekstra': os.path.join(UPLOAD_BASE, 'ekstra')
}
...
def index():
    with get_db() as conn:
        berita = conn.execute("SELECT id, judul FROM berita ORDER BY id DESC LIMIT 5").fetchall()
...
def pendaftaran():
    if request.method == 'POST':
        ...
        with get_db() as conn:
            conn.execute("INSERT INTO pendaftaran (nama, asal_sekolah, no_hp) VALUES (?, ?, ?)",
                         (nama, asal, hp))
...
if __name__ == '__main__':
    ...
    if not os.path.exists(DB_PATH):
        print("Database belum ada! Jalankan: python create_tables.py")
